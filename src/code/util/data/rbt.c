...
rbnode *rb_insert(rbtree *rbt, void *data)
{
	 ...
	 */
	if (current->parent->color == RED_RBT) {
...
void insert_repair(rbtree *rbt, rbnode *current)
{
	...
	do {
		...
		if (current->parent == current->parent->parent->left) {
			...
			if (uncle->color == RED_RBT) {
				/* insertion into 4-children cluster */

				/* split */
				current->parent->color = BLACK_RBT;
				uncle->color = BLACK_RBT;
				...
				current->parent->color = BLACK_RBT; /* thus goto break */
				current->parent->parent->color = RED_RBT;
	...
	} while (current->parent->color == RED_RBT);
}
...
void *rb_delete(rbtree *rbt, rbnode *node, int keep)
{
	 ...
	 */
	if (target->color == BLACK_RBT) {
		if (child->color == RED_RBT) {
			/* deletion from 3-children cluster (BLACK target node, RED child node) */
			child->color = BLACK_RBT;
...
void delete_repair(rbtree *rbt, rbnode *current)
{
	...
	do {
		if (current == current->parent->left) {
			...
			if (sibling->right->color == BLACK_RBT && sibling->left->color == BLACK_RBT) {
				/* 2-children sibling cluster, fuse by recoloring */
				sibling->color = RED_RBT;
				if (current->parent->color == RED_RBT) { /* 3/4-children parent cluster */
					current->parent->color = BLACK_RBT;
					break; /* goto break */
				...
				if (sibling->right->color == BLACK_RBT) {
					sibling->left->color = BLACK_RBT;
					sibling->color = RED_RBT;
			...
			if (sibling->right->color == BLACK_RBT && sibling->left->color == BLACK_RBT) {
				...
				/* perform an adjustment (3-children sibling cluster has two representations) */
				if (sibling->left->color == BLACK_RBT) {
					sibling->right->color = BLACK_RBT;
					sibling->color = RED_RBT;
...
{
	if (RB_ROOT(rbt)->color == RED_RBT || RB_FIRST(rbt)->color == RED_RBT || RB_NIL(rbt)->color == RED_RBT)
		return 0;
...
int check_black_height(rbtree *rbt, rbnode *n)
{
	...
	if (n->color == RED_RBT && (n->left->color == RED_RBT || n->right->color == RED_RBT || n->parent->color == RED_RBT))
		return 0;
	...
	return lbh + (n->color == BLACK_RBT ? 1 : 0);
}
