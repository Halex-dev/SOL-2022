...
void hashmap_set(hashmap* m, void* key, size_t ksize, void* val)
{
...
bool hashmap_get_set(hashmap* m, void* key, size_t ksize, void** out_in)
{
	if (m->count + 1 > HASHMAP_MAX_LOAD * m->capacity)
		hashmap_resize(m);
...
void hashmap_set_free(hashmap* m, void* key, size_t ksize, void* val, hashmap_callback c, void* usr)
{
	if (m->count + 1 > HASHMAP_MAX_LOAD * m->capacity)
		hashmap_resize(m);
...
bool hashmap_get(hashmap* m, void* key, size_t ksize, void** out_val)
{
	uint32_t hash = hash_data(key, ksize);
	struct bucket* entry = find_entry(m, key, ksize, hash);
