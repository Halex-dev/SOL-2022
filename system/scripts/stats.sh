...
if [ $# -eq 0 ]; # no additional argument => using last log file in ./logs/
  then
    if [ -d system/logs/server ];
      then
        LOG_FILE=system/logs/server/$(ls ./system/logs/server/ | grep -E 'log-*' | tail -n1) || { echo "No log file could be found inside system/logs/server/ directory. Aborting script."; exit 1; }  
    else # log
...
echo "No. of successful READ_FILE: ${n_reads}"
echo "No. of bytes read by READ_FILE: ${tot_bytes_read}"
if [ ${n_reads} -gt 0 ]; then
    mean_bytes_read=$(echo "scale=4; ${tot_bytes_read} / ${n_reads}" | bc -l)
    echo "Mean number of bytes read by READ_FILE: ${mean_bytes_read}"
fi

echo ""

echo "No. of successful READ_N_FILES: ${n_readNFiles}"
echo "No. of bytes read by READ_N_FILES: ${tot_bytes_readNFiles}"
if [[ ${n_readNFiles} -gt 0 ]]; then
    mean_bytes_readNFiles=$(echo "scale=4; ${tot_bytes_readNFiles} / ${n_readNFiles}" | bc -l)
    echo "Mean number of bytes read by READ_N_FILES: ${mean_bytes_readNFiles}"
fi
...
echo "No. of successful WRITE_FILE: ${n_writes}"
echo "No. of bytes written by WRITE_FILE: ${tot_bytes_writes}"
if [ ${n_writes} -gt 0 ]; then
    mean_bytes_written=$(echo "scale=4; ${tot_bytes_writes} / ${n_writes}" | bc -l)
    echo "Mean number of bytes written by WRITE_FILE: ${mean_bytes_written}"
fi

echo ""

echo "No. of successful APPEND_TO_FILE: ${n_appends}"
echo "No. of bytes written by APPEND_TO_FILE: ${tot_bytes_appends}"
if [[ ${n_appends} -gt 0 ]]; then
    mean_bytes_appends=$(echo "scale=4; ${tot_bytes_appends} / ${n_appends}" | bc -l)
    echo "Mean number of bytes written by APPEND_TO_FILE: ${mean_bytes_appends}"
fi
